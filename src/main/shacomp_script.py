...
def main(sha_path: Path, data_path: Path):
    ...
    if do_sort:
        ...
        for f in sha_files1:
            helper.sort_sha(os.path.join(sha_path, f))
    ...
    if os.path.isfile(junk_sha_filename):
        helper.load_sha_set(sha_set=lists.sha_blacklist, filename=junk_sha_filename)
    ...
    # global_dict = shacomp_rw.read_write(dir_name=sha_path, master=master, save_uniques_list=False, read_junks=False, sha_blacklist={zero_sha})
    global_dict = shacomp_rw.read_write(
        dir_name=sha_path,
        master=master,
        save_uniques_list=False,
    ...
    if check_non_consistent_exts_keys:
        non_consistent_exts_keys = shacomp_rw.do_all_dups_have_same_ext(
            global_dict, ext_whitelist=lists.ext_whitelist, sha_blacklist=lists.sha_blacklist
        )
    ...
    if do_look_for_missing_and_mismatch:
        shacomp_rw.look_for_missing_and_mismatch(global_dict, data_dir_name=data_path, log_path=sha_path, verify_sha=False)
    ...
    if prepare_delete_list:
        ...
        shacomp_rw.delete_duplicates(
            d=global_dict,
            junk_sha_filename_tup_list=sha_filename_to_delete_tup_list,
            delete_junk_filename_templates=True,
            sha_to_delete_set=sha_to_delete_set,
            data_dir=data_path,
            verify_sha=True,
        )
        helper.save_sha_set(sha_to_delete_set, junk_sha_filename)

    helper.save_sha_tup_list(sha_filename_to_delete_tup_list, os.path.join(sha_path, "_deleted.sha512"))
    actually_delete = False
    do_delete = False
    remove_log_filename = "deleted_files_log.txt"
    if do_delete:
        shacomp_rw.delete_from_list(
            sha_filename_to_delete_tup_list=sha_filename_to_delete_tup_list,
            data_dir=data_path,
            actually_delete=actually_delete,
            remove_log_filename=remove_log_filename,
        )

    plot_dict.dict_stats(global_dict, do_plot=False)
...
def main1():
    ...
    sha_dir = r"../../temp"
    sha_path = Path(helper.normpath1(os.path.join(pathname, sha_dir)))
    data_path = Path("p:\\")
    main(sha_path=sha_path, data_path=data_path)
